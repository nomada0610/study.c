...
void CheckContact(contact*);

void LoadContact(contact* p)
{
	PeoInfo tmp = { 0 };
	FILE* Readpf = fopen("contact.dat", "rb");

	while (fread(&tmp, sizeof(PeoInfo), 1, Readpf))
	{
		CheckContact(p);
		//memcpy(&p->data[p->size], &tmp, sizeof(PeoInfo));
		p->data[p->size] = tmp;
		p->size++;
	}
...
{
	int i = 0;
	FILE* Writepf = fopen("contact.dat", "wb");
	if (!Writepf)
	{
		printf("SAVE ERROR: %s\n", strerror(errno));
		return;
	...
	for (i = 0; i < p->size; i++)
	{
		fwrite(&(p->data[i]), sizeof(PeoInfo), 1, Writepf);
	}
	fclose(Writepf);
	Writepf = NULL;
}
